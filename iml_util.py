...
def load_csv(test_subject, trial, folder, file):
    ...
    file_path = os.path.abspath(f'./sessions/{test_subject}/{trial}/{folder}/{file}.csv')
...
def load_csv_0(test_subject, folder, filename):
    ...
    Returns:
        data (pd.DataFrame): The loaded CSV file as a pandas DataFrame.
    """
    file_path = os.path.join(os.path.relpath(f'./sessions/test_subject_0/gaze/' + folder), f'{filename}.csv')
...
def gen_data_subject_0():
    ...
    for f in range(4):
        ...
        images = [open_img(os.path.abspath(f'sessions/test_subject_0/gaze/{f}'), i) for i in range(len(positions)-1)]
        D.append({
...
def gen_data_subject(test_subject, trial):
    ...
    for f in p:
        path = f# + "/frames"
        D.append({
            f"pupils_train": create_pupil_data(load_csv(test_subject, trial, path, 'mean_pupil_coordinates')),
            f"pupils_test": create_pupil_data(load_csv(test_subject, trial, path, 'mean_pupil_coordinates')),
